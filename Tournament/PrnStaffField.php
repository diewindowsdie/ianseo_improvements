...
$currentSessionCompetitionOfficials = [];
...
if (isset($_REQUEST['TourId'])) {
    ...
    $currentSessionCompetitionOfficials = $_SESSION;
...
$Select = "
	...
	ORDER BY TiIsSigningProtocols desc, ItId IS NOT NULL, ItOrder, TiAccreditation asc, TiName, TiGivenName ASC";
...
while ($judge = safe_fetch($resultSet)) {
    ...
    $pdf->Cell(62, $rowHeight, trim($judge->TiName) . ' ' . trim($judge->TiGivenName) . ($judge->TiLastName ? ' ' . trim($judge->TiLastName) : ""), 1, 0, 'L');
...
//если просят конкретное соревнование - восстановим $_SESSION, который перетирается на время выполнения скрипта
    if (isset($_REQUEST['TourId'])) {
        $_SESSION = $currentSessionCompetitionOfficials;
    }
}
