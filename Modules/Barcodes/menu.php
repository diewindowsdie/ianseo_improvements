...
if($on) {
	...
	if(!empty($ret['ELIM']) AND subFeatureAcl($acl,AclEliminations,'') == AclReadWrite) {
	...
	if(!empty($ret['ELIMP']) AND subFeatureAcl($acl,AclEliminations,'') == AclReadWrite) {
	...
	if(!empty($ret['FINI']) AND subFeatureAcl($acl,AclIndividuals,'') == AclReadWrite) {
	...
	if(!empty($ret['FINT'])  AND subFeatureAcl($acl,AclTeams,'') == AclReadWrite) {
	...
	if(!empty($ret['ROBIN'])  AND subFeatureAcl($acl,AclRobin,'') == AclReadWrite) {
	...
	if(!empty($ret['FINT'])  AND subFeatureAcl($acl,AclTeams,'') == AclReadWrite and !empty($_SESSION['TourLocSubRule']) and $_SESSION['TourLocSubRule']=='SetFRChampsD1DNAP') {
