 ...
 * </ul>
 */
class Obj_Rank_Robin_3_SetFRD12026 extends Obj_Rank{
	public $PFilter='';
	public $MFilter='';
	...
	*/
	...
	public function read(){
		$PFilter=$this->safeFilter('Participants');
		$TourDates=[];
		...
		if(!$this->OnlyMatch) {
			...
			$SQL="select Participants.*,
				...
				left join (
				    select group_concat(concat_ws('|',TqdDistance, TqdScore, TqdBonus, TqdRank) order by TqdDistance separator '-') as Distances, TqdCoId, TqdSubTeam, TqdEvent
				    from TeamQualificationDistances
				    where TqdTournament={$this->tournament}
				    group by TqdCoId, TqdSubTeam, TqdEvent) tqd on TqdCoId=RrPartParticipant and TqdSubTeam=RrPartSubTeam and TqdEvent=RrPartEvent
				left join (
			...
			$this->data['meta']['stages']=	getModuleParameter('FFTA', 'D1TourDates', ['D1' => [], 'D2' => [], 'D3' => []], $this->tournament);
			...
			while($myRow=safe_fetch($q)) {
                ...
                $Distances=explode("-",$myRow->Distances);
                $Quals=[];
                foreach($Distances as $Distance) {
                    $tmp=explode("|",$Distance);
                    $Quals[$tmp[0]]=[
                        'distance' => $tmp[0],
                        'score' => $tmp[1],
                        'bonus' => $tmp[2],
        ...
        // step 3, add the qualifications values and bonus
        $q=safe_r_sql("select *
            from TeamQualificationDistances
            where TqdTournament={$this->tournament}");
