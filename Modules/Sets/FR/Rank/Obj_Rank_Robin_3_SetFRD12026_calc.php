 ...
 * Estende Obj_Rank
 */
class Obj_Rank_Robin_3_SetFRD12026_calc extends Obj_Rank_Robin_3_SetFRD12026 {
	public function calculate() {
        ...
        // in D1 2026, the rank is made adding the bonus given in all distances
		$q=safe_r_sql("select RrMatchAthlete, RrMatchSubTeam, sum(RrMatchRoundPoints) + Bonus as Points, sum(RrMatchTieBreaker) as TieBreaker1, Score as TieBreaker2
			from RoundRobinMatches
			inner join (
			    select sum(TqdBonus) as Bonus, sum(TqdScore) as Score, TqdCoId, TqdSubTeam, TqdEvent
			    from TeamQualificationDistances
			    where TqdTournament={$this->tournament}
			    group by TqdCoId, TqdSubTeam, TqdEvent
			) TeamQualificationDistances on TqdCoId=RrMatchAthlete and TqdSubTeam=RrMatchSubTeam and TqdEvent=RrMatchEvent
			...
			order by Points desc, TieBreaker1 desc, TieBreaker2 desc");
		$rank=0;
		$i=0;
		$oldStatus='';
		while($r=safe_fetch($q)) {
			...
			if($oldStatus!="{$r->Points}-{$r->TieBreaker1}-{$r->TieBreaker2}") {
				$rank+=$i;
				$i=0;
			}
			$oldStatus="{$r->Points}-{$r->TieBreaker1}-{$r->TieBreaker2}";
			if($Team) {
				safe_w_sql("UPDATE Teams
					SET TeRankFinal=$rank, TeScore=$r->Points, TeGold=$r->TieBreaker1, TeXnine=$r->TieBreaker2, TeTimestampFinal=TeTimestampFinal
	...
	public function calculateGroup() {
		...
		while($r=safe_fetch($q)) {
			...
			safe_w_sql("update RoundRobinParticipants 
				set RrPartPoints=$r->Points, RrPartTieBreaker=$r->TieBreaker1, RrPartTieBreaker2=$r->TieBreaker2, RrPartGroupRank=$rank, RrPartGroupRankBefSO=$rank 
