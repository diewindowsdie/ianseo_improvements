...
$EventsToPrint=array_keys($rankData['sections']);
if(!empty($rankData['TeamsFinalRank'])) {
	$FromRobin=true;
	$OldRankData=$rankData;
	$PdfData->rankData=$rankData['TeamsFinalRank'];
	include(__DIR__.'/RankTeam_3_SetFRD12026.inc.php');

	// $pdf->AddPage();
	$PdfData->rankData=$OldRankData;
    $EventsToPrint=array_keys($rankData['TeamsFinalRank']['sections']);
}
...
if($EventsToPrint) {
	...
	$OldTitle='';
	foreach($EventsToPrint as $IdEvent) {
        $section=$rankData['sections'][$IdEvent];
		if(empty($Desc)) {
