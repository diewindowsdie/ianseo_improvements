	...
	if(!empty($section['meta']['hasChildren'])) {
		if($section['meta']['parent']=='') {
			...
			if(count($section['items'])) {
                ...
                $wTS=9;
                $wTP=9;
                $wT=$pdf->getPageWidth() - 26 - ($wTP+$wTS)*2 - $wTQ - ($wB+$wR+$wQ)*$Blocks;
				...
				foreach($section['items'] as $item) {
					...
					if($NeedTitle) {
                        ...
                        // print the stages
                        $pdf->setX(10+$wT+6+($wTP+$wTS)*2 + $wTQ);
                        switch($Blocks) {
                            case 1:
						...
						$pdf->Cell(6, 5, $section['meta']['fields']['rank'], 'LTB', 0, 'C', 1);
					...
					$pdf->Cell(6, 4, ($item['rank'] ? $item['rank'] : ''), 'TBL', 0, 'C', 0);
