...
// 	$ret[]='<div class="Title">'.$data['meta']['descr'].$Title2Rows.$RecTitle .'</div>';
	...
	foreach($data['items'] as $key => $archer) {
		...
		foreach ($RecTot as $RecCode => $Record) {
			if($archer['irm']<10 and !$Final and $Record['gap'] and $archer['recordGap'] < $Record['tot'] and
				(!$Record['claim']
...
function getPageDefaults(&$RMain) {
	...
	$ret=array(
		...
		'Rank' => 'flex: 0 0 4.5vw; text-align:right;',
		...
		'CountryDescr' => 'flex: 1 1 20vw;white-space:nowrap;overflow:hidden;font-size: 1.15vw;',
		...
		'DistPos' => 'flex: 0 0 1.5vw; text-align:left; font-size:1vw;',
