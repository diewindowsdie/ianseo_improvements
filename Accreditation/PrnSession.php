...
$atSql = createAvailableTargetSQL(($_REQUEST["Session"]??0));

$MyQuery = "SELECT EnCode as Bib, EnName AS Name, FirstName, FullTgtSession AS Session, CONCAT(FullTgtTarget,FullTgtLetter) AS TargetNo, CoCode AS NationCode, CoName AS Nation, EnClass AS ClassCode, EnDivision AS DivCode, EnAgeClass as AgeClass, EnSubClass as SubClass, EnStatus as Status, EnIndClEvent AS `IC`, EnTeamClEvent AS `TC`, EnIndFEvent AS `IF`, EnTeamFEvent as `TF`, EnTeamMixEvent as `TM`, if(AEId IS NULL, 0, 1) as OpDone, EnPays, APPrice ";
$MyQuery.= "FROM ($atSql) at ";
$MyQuery.= "LEFT JOIN ";
$MyQuery.= "(SELECT QuSession, QuTarget, QuLetter, EnId, EnCode, EnName, upper(EnFirstName) AS FirstName, CoCode, CoName, EnClass, EnDivision, EnAgeClass, EnSubClass, EnStatus, EnIndClEvent, EnTeamClEvent, EnIndFEvent, EnTeamFEvent, EnTeamMixEvent, EnPays, APPrice, AEId ";
...
$MyQuery.= ") as Sq ON QuSession=FullTgtSession AND QuTarget=FullTgtTarget AND QuLetter=FullTgtLetter ";
if(isset($_REQUEST["Session"]) && is_numeric($_REQUEST["Session"]))
    $MyQuery .= "WHERE FullTgtSession = " . StrSafe_DB($_REQUEST["Session"]) . " ";
...
//Disegna i Pallini per la partecipazione
    $pdf->DrawParticipantDetails(intval($MyRow->IC), intval($MyRow->IF), intval($MyRow->TC), intval($MyRow->TF), intval($MyRow->TM), intval($MyRow->OpDone));
    if ($MyRow->OpDone != 0)
        $pdf->SetAccreditedColor();
    else
        $pdf->SetDefaultColor();
