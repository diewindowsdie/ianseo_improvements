...
while ($MyRow=safe_fetch($Rs)) {
	...
	if($MyRow->EnTournament and ($CurrentTour!=$MyRow->EnTournament or $CurrentBadge!=$MyRow->IcNumber)) {
		...
		$RndImages=array();
		$q=safe_r_sql("select * from IdCardElements where IceTournament={$MyRow->EnTournament} and IceCardType='$CardType' and IceCardNumber=$MyRow->IcNumber and IceType='RandomImage' order by IceCardPage, IceOrder");
		while($r=safe_fetch($q)) {
			$r->Options=unserialize($r->IceOptions);
			$RndImages[$r->IceCardPage][]=$r;
		}
    ...
    $MaxPage=max(count($RndImages), count($Elements));
    foreach(range(1,$MaxPage) as $Page) {
        ...
        foreach(($Elements[$Page]??[]) as $Element) {
            unset($Text);
