...
if (!isset($_REQUEST["id"]) || getModuleParameter(RESULTS_PUBLICATION_MODULE_NAME, COMPETITION_HIDDEN_PARAM_NAME, "0", $_REQUEST["id"]) === "1") {
    exitNotFound();
}
...
$_REQUEST["TourId"] = $_REQUEST["id"];

switch ($_REQUEST["report"]) {
    case 'qI':
        $_REQUEST["Event"] = getVisibleEventCodesConsideringTeams($_REQUEST["id"], "0");

        require_once "../../../Qualification/PrintIndividual.php";
        break;
    case 'qT':
        $_REQUEST["Event"] = getVisibleEventCodesConsideringTeams($_REQUEST["id"], "1");
    ...
    case 'rT':
        ...
        break;
    case "qcI":
        if (getModuleParameter(RESULTS_PUBLICATION_MODULE_NAME, PUBLISH_SCORECARDS_PARAM_NAME, "1", $_REQUEST["id"]) === "0") {
            exitNotFound();
        }
        ...
        $maxDistancesQuery = "select max((QuD1Arrowstring != '') + (QuD2Arrowstring != '') + (QuD3Arrowstring != '') + (QuD4Arrowstring != '') +
                                ...
                                where e.EnTournament = " . StrSafe_DB($_REQUEST["id"]) . " and q.QuSession = " . $_REQUEST["session"];
    ...
    case "sac":
        ...
        $_REQUEST["TeamEvents"] = getVisibleEventCodesConsideringTeams($_REQUEST["id"], "0");
    ...
    case "fs":
        ...
        $_REQUEST["ses"] = getModuleParameter(RESULTS_PUBLICATION_MODULE_NAME, SHOW_FINAL_SESSIONS_IN_SCHEDULE_PARAM_NAME, array(), $_REQUEST["id"]);
    ...
    case "p":
        ...
        $_REQUEST["Events"] = getVisibleEventCodesConsideringTeams($_REQUEST["id"], "0");
        $_REQUEST["TeamEvents"] = getVisibleEventCodesConsideringTeams($_REQUEST["id"], "1");

        $c = new stdClass();
        $c->ToId = $_REQUEST["id"];

        if (getModuleParameter(PROTOCOL_MODULE, Checked1ParameterName, "0", $_REQUEST["id"]) === "1") {
            $_REQUEST["country1"] = "1";
            $_REQUEST["StatHeader1"] = getRegionStatisticsHeaderParameter($c, "1");
        }
        if (getModuleParameter(PROTOCOL_MODULE, Checked2ParameterName, "0", $_REQUEST["id"]) === "1") {
            $_REQUEST["country2"] = "1";
            $_REQUEST["StatHeader2"] = getRegionStatisticsHeaderParameter($c, "2");
        }
        if (getModuleParameter(PROTOCOL_MODULE, Checked3ParameterName, "0", $_REQUEST["id"]) === "1") {
            $_REQUEST["country3"] = "1";
