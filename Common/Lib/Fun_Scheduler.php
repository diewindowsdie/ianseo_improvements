	...
	function GetSchedule() {
		...
		if(!$this->SesType or strstr($this->SesType, 'Z')) {
			$SQL[]="select distinct
				...
				left join Session on SesTournament=SchTournament and (SesLocation='' or SesLocation=SchLocation) and SesDtStart<=concat(SchDay, ' ', SchStart) and SesDtEnd>=date_add(concat(SchDay, ' ', SchStart), interval SchDuration minute)
		...
		if(!$this->SesType or strstr($this->SesType, 'F')) {
			...
			$SQL[]="select distinct
				...
				left join Session on SesTournament=FsTournament and SesType='F' and (SesEvents='' or find_in_set(concat(FsTeamEvent,FsEvent), SesEvents)) and concat(FSScheduledDate,' ',FSScheduledTime) between SesDtStart and SesDtEnd
	...
	function getScheduleByDay() {
        ...
        foreach($this->GetSchedule() as $Date => $SesGroups) {
            foreach($SesGroups as $SesGroup => $Times) {
                ...
                $ret[]=array('Day'=>$Date.($SesGroup?" - $SesGroup":''), 'Items'=>$tmpToday);
            }
        }
        return $ret;
    }
	...
	function getSchedulePDF(&$pdf='') {
		...
		foreach($this->GetSchedule() as $Date => $SesGroups) {
            foreach($SesGroups as $SesGroup => $Times) {
                ...
                $pdf->Cell(0, $CellHeight, formatTextDate($Date, true) . ($SesGroup? " - $SesGroup" :''),1,1,'L',1);
                            ...
                            foreach($Items as $k => $Item) {
                                ...
                                if($Item->Type=='Z') {
									...
									if(!$IsTitle) {
                                        ...
                                        $fill = 0;
                                        $cellWidth = $descrSize;
                                        $text = strip_tags($Item->Title).(($Item->SubTitle or $Item->Text or !$Item->RowLocation or $this->NoLocations) ? '' : ' ('.$Item->RowLocation.')');
                                        if ($Item->UID == "") {
                                            $pdf->SetFont('', 'B');
                                            $pdf->SetFontSize(10);
                                            $pdf->SetX($StartX + 5);
									...
									if(!$IsTitle) {
										...
										$pdf->Cell($titleDescrSize, $titleCellHeight, $mainText.(($Item->SubTitle or $Item->Text or !$Item->RowLocation or $this->NoLocations) ? '' : ' ('.$Item->RowLocation.')'), 0, 1, $titleAlign, $fill);
										$pdf->SetFont('', '');
                                        $pdf->SetFontSize(8);
										$RepeatTitle=$Item->Title;
									}
								    ...
								    if($OldSubTitle!=$Item->SubTitle && $Item->Title != $mainText) {
                                        ...
                                        $pdf->SetX($StartX+$TimeColumns);
                                        $pdf->SetFont('', 'BI');
    									$pdf->Cell($descrSize, $CellHeight, $Item->Title.(($Item->Text or !$Item->RowLocation or $this->NoLocations) ? '' : ' ('.$Item->RowLocation.')'), 0, 1, 'L', 0);
                                        $pdf->SetFont('', '');
                                        $OldSubTitle=$Item->SubTitle;
                                        $IsTitle=false;
                                    }
...
// 		$this->Ods->currentRow=-1;
		...
		foreach($this->Schedule as $Date => $SesGroups) {
            foreach($SesGroups as $SesGroup => $Times) {
                ...
                $this->Ods->addRow(formatTextDate($Date, true).($SesGroup?" - $SesGroup":''));
    ...
    // 											debug_svela($Item);
		...
		foreach($this->Schedule as $Date => $SesGroups) {
            foreach($SesGroups as $SesGroup => $Times) {
                $this->Ods->setActiveSheet($Date.($SesGroup?" - $SesGroup":''));
...
// 			array(204,0,255),
		...
		foreach($this->GetSchedule() as $Date => $SesGroups) {
            foreach($SesGroups as $SesGroup => $Times) {
                $FOP[$SesGroup][$Date]=array('min'=>0, 'max'=>0, 'times'=>array());
                ksort($Times);
