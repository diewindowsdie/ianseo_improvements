	...
	function getSchedulePDF(&$pdf='') {
		...
		foreach($this->GetSchedule() as $Date => $SesGroups) {
            foreach($SesGroups as $SesGroup => $Times) {
                ...
                $pdf->Cell(0, $CellHeight, formatTextDate($Date, true) . ($SesGroup? " - $SesGroup" :''),0,1,'L',1);
                        ...
                        foreach($Distances as $Distance => $Items) {
                            foreach($Items as $k => $Item) {
                                ...
                                if(!$pdf->SamePage(1, $CellHeight, '', false)) {
                                    ...
                                    // Day...
                                    $pdf->SetFont('', 'B');
                                    $pdf->Cell(0, $CellHeight, formatTextDate($Date, true) . ($SesGroup? " - $SesGroup" :'') . '    ('.get_text('Continue').')',0,1,'L',1);
                                    $FirstTitle=true;
