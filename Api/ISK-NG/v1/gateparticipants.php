...
if(empty($Options)) {
    $res = array('action' => 'gateparticipants', 'error' => 1, 'apiVersion' => $req->apiVersion, 'device' => $req->device);
    return;
}
...
while($r=safe_fetch($q)) {
    ...
    $Template['hash'] = md5($Template['entryCode'].$Template['hasPhoto'].$Template['familyName'].$Template['givenName'].$Template['countryCode'].$Template['countryName'].$Template['caption'].implode(',',$Template['zones']).$Template['extras']);
...
foreach ($Options as $ToId=>$Sessions) {
    if(!empty($Sessions)) {
        ...
        if(array_key_exists($ToCode,$regexpList)) {
            $tmpKeyStr = "ToCode,'|',IFNULL(localbib.EdExtra,EnCode)" . ($regexpList[$ToCode]["country"] != -1 ? ",'|',CoCode" : "") . ($regexpList[$ToCode]["division"] != -1 ? ",'|',EnDivision" : "");
        }
        $SQL = "
            (
